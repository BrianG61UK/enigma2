...
def LoadPixmap(path, desktop=None, cached=None, width=0, height=0, scaletoFit=0, align=RT_HALIGN_CENTER):
	if path[-4:] == ".png":
		...
		ptr = loadPNG(path, 0, 0)	#Brian was here
	elif path[-4:] == ".jpg":
		# don't cache unless caller explicity requests caching
		ptr = loadJPG(path, 0)	#Brian was here
	elif path[-4:] == ".svg":
		...
		# don't cache unless caller explicity requests caching
		ptr = loadSVG(path, 0, width, height, scale, scaletoFit, align) #Brian turned off all caching
	elif path[-1:] == ".":
